...
字典为准；计算结果来自逐网格汇总数据和后处理图像。四边形一阶迎风组还保留了
完整的运行日志和网格检查文件，其余实验组当前快照主要保留汇总数据、时间历史和
图像。因此，后文会明确区分“数据级证据”和“完整运行证据”。
...
| 完成求解器开发与验证 | OpenFOAM 14 求解器、自动化脚本和数据后处理 | 第 3、5 节 |
...
| 验收项 | 标准 |
|---|---|
| 求解器编译 | 生成 `build/01_advection_equation/bin/explicitAdvectionFoamStudent` |
...
OpenFOAM 接口对应关系，主要参考项目中的题目解答文档
`../../pdf/题目解答.pdf`，并结合学生版求解器源码和实际案例字典进行核对。
本报告将数学步骤与实际求解器、案例字典和运行结果连接起来。
...
$$\phi_f=\begin{cases}\phi_{\mathrm{owner}}, & F_f\geq 0,\\\phi_{\mathrm{neighbour}}, & F_f<0.\end{cases}$$
...
$$R_c=\frac{1}{V_c}\sum_fF_{cf}\phi_f.$$

### 3.3 时间离散

采用前向 Euler 格式：

$$\phi_c^{n+1}=\phi_c^n-\Delta t\frac{1}{V_c}\sum_fF_{cf}^n\phi_f^n.$$

代码中没有调用 `fvm::div`、`fvm::ddt` 或线性方程组求解器，因此该求解器确实采用
显式时间和显式空间离散。

前向 Euler 时间离散本身为一阶精度。由于本项目在网格加密时保持目标 CFL 基本不变，
而对流速度固定，时间步长随网格尺度近似按 $\Delta t\propto h$ 缩放。因此第 6 节给出的
观察收敛阶反映空间误差与时间误差的联合行为，不能单独作为空间离散格式理论阶数的严格
证明。若要独立测定空间阶数，还需要在每个网格上进一步进行时间步加密，使时间误差低于
空间误差。
...
$$\Delta t=\frac{2\,\mathrm{Co}_{\max}}{\max_c(r_c)}.$$
...
对于正常的 CFL 步，程序按上述公式选择时间步；为准确到达终止时间，最后一步使用
剩余时间与 CFL 候选步长中的较小值。因此实际最大 CFL 应满足 `maxCo <= 0.2`，
而最后一个被截短的时间步可能小于 `0.2`。表中显示的 `0.200` 是各案例时间历史中的
最大值，并不表示每一个时间步都严格等于 `0.2`。
...
$$\phi(x,y,t)=\sin\left(2\pi(x+y-2t)\right).$$

当 $t=1$ 时：

$$\phi(x,y,1)=\sin\left(2\pi(x+y-2)\right)=\sin\left(2\pi(x+y)\right).$$
...
$$\boldsymbol{u}(x,y)=\left(0.5-y,\;x-0.5,\;0\right).$$
...
$$L_1=\frac{\sum_c|\phi_c-\phi_c^{\mathrm{exact}}|V_c}{\sum_c|\phi_c^{\mathrm{exact}}|V_c}.$$
...
表中“最终振幅”采用后处理中的定义
`(max(T)-min(T))/2`，不是“最大绝对值”。对于本组正弦波，数值场近似关于零对称，
所以二者数值上接近，但指标定义仍应区分。
...
<figure id="fig-01">
  <img src="../../figures/01_advection_equation/analysis/01_sine_wave_quad/convergence_errors.png" alt="四边形一阶迎风误差收敛曲线" width="720">
  <figcaption>图 1：四边形一阶迎风误差收敛曲线</figcaption>
</figure>

该图展示 $N=10,20,40,80$ 时归一化误差随网格尺度变化的关系；横坐标为名义网格尺度，
纵坐标为误差，数据来源为
`data/01_advection_equation/analysis/01_sine_wave_quad/convergence_summary.csv`。如[图 1](#fig-01)所示，误差随网格加密整体下降。

<figure id="fig-02">
  <img src="../../figures/01_advection_equation/analysis/01_sine_wave_quad/convergence_order.png" alt="四边形一阶迎风收敛阶" width="720">
...
可以看到，误差随网格加密单调下降，$L_1$ 观察阶由 `0.275888`、`0.544977`
增加到 `0.745023`，呈现向一阶联合收敛行为靠近的趋势。这里的“一阶”同时受到
一阶迎风空间离散和按固定 CFL 缩放的前向 Euler 时间误差影响，不能解释为只验证了
空间格式的阶数。低分辨率下误差较大，主要原因是一阶迎风格式的数值耗散在粗网格上
非常明显。
...
<figure id="fig-03">
  <img src="../../figures/01_advection_equation/cases/01_sine_wave_quad/N80/field_comparison.png" alt="四边形一阶迎风场对比 N80" width="720">
...
<figure id="fig-04">
  <img src="../../figures/01_advection_equation/cases/01_sine_wave_quad/N80/diagonal_profile.png" alt="四边形一阶迎风对角线剖面 N80" width="720">
...
<figure id="fig-05">
  <img src="../../figures/01_advection_equation/cases/01_sine_wave_quad/N80/amplitude_history.png" alt="四边形一阶迎风振幅历史 N80" width="720">
...
<figure id="fig-06">
  <img src="../../figures/01_advection_equation/cases/01_sine_wave_quad/N80/cfl_history.png" alt="四边形一阶迎风 CFL 历史 N80" width="720">
  <figcaption>图 6：四边形一阶迎风 CFL 历史 N80</figcaption>
</figure>

CFL 历史中的最大值为 `0.2`，说明该组误差和振幅变化是在相同的 CFL 上限条件下产生的；
末步若为到达终止时间而被截短，其 CFL 可以低于该值。
...
<figure id="fig-07">
  <img src="../../figures/01_advection_equation/analysis/03_sine_wave_tri_upwind/convergence_errors.png" alt="三角形一阶迎风误差收敛曲线" width="720">
...
<figure id="fig-08">
  <img src="../../figures/01_advection_equation/analysis/03_sine_wave_tri_upwind/convergence_order.png" alt="三角形一阶迎风收敛阶" width="720">
  <figcaption>图 8：三角形一阶迎风收敛阶</figcaption>
</figure>

三角形的 $L_1$ 观察收敛阶从 `0.352821`、`0.613558` 增加到 `0.788683`，
呈现向一阶联合收敛行为靠近的趋势，与四边形案例相近。该结果不能单独视为空间
离散阶数的严格证明，差别还受到局部面方向、单元数量、对流方向和时间误差的共同影响；
对应的观察阶见[图 8](#fig-08)。

<figure id="fig-09">
  <img src="../../figures/01_advection_equation/analysis/03_sine_wave_tri_upwind/all_N_comparison.png" alt="三角形一阶迎风全分辨率场对比" width="720">
...
<figure id="fig-10">
  <img src="../../figures/01_advection_equation/cases/03_sine_wave_tri_upwind/N80/field_comparison.png" alt="三角形一阶迎风场对比 N80" width="720">
...
<figure id="fig-11">
  <img src="../../figures/01_advection_equation/cases/03_sine_wave_tri_upwind/N80/diagonal_profile.png" alt="三角形一阶迎风对角线剖面 N80" width="720">
...
<figure id="fig-12">
  <img src="../../figures/01_advection_equation/cases/03_sine_wave_tri_upwind/N80/amplitude_history.png" alt="三角形一阶迎风振幅历史 N80" width="720">
...
<figure id="fig-13">
  <img src="../../figures/01_advection_equation/cases/03_sine_wave_tri_upwind/N80/cfl_history.png" alt="三角形一阶迎风 CFL 历史 N80" width="720">
  <figcaption>图 13：三角形一阶迎风 CFL 历史 N80</figcaption>
</figure>

CFL 历史中的最大值不超过目标值 `0.2`，说明三角形网格导入后面通量、单元体积和时间步
计算之间仍然匹配。因此该组与四边形组的差异主要来自网格几何和空间离散，而不是 CFL
上限不同。
...
这里的“最终振幅”仍定义为最终场的半峰峰值
`(max(T)-min(T))/2`；本组数值场近似关于零对称，所以它与最大绝对值相同。
连续初始场的理论振幅为 `1`，而粗网格单元中心采样得到的离散初始振幅可能略小于 `1`。
因此四边形线性迎风在 $N=10,20,40,80$ 时的最终振幅分别为 `1.588312`、`1.417537`、
`1.211436` 和 `1.102974`，相对于连续初始振幅 `1` 的过冲约为 `58.8%`、`41.8%`、
`21.1%` 和 `10.3%`。网格加密会减小过冲，但当前未限制的线性重构仍没有完全恢复
有界性。

<figure id="fig-14">
  <img src="../../figures/01_advection_equation/analysis/02_sine_wave_quad_linearUpwind/convergence_errors.png" alt="四边形线性迎风误差收敛曲线" width="720">
...
<figure id="fig-15">
  <img src="../../figures/01_advection_equation/analysis/02_sine_wave_quad_linearUpwind/convergence_order.png" alt="四边形线性迎风收敛阶" width="720">
  <figcaption>图 15：四边形线性迎风收敛阶</figcaption>
</figure>

当 $N=40,80$ 时，观察收敛阶分别为 `1.030879` 和 `1.048551`，在当前空间—时间
联合误差定义下接近一阶；[图 15](#fig-15)显示了这一趋势。
粗网格的阶数不稳定，是因为此时正弦波还没有被充分分辨，误差中同时包含耗散、相位
误差和线性重构误差，不能把单个区间的斜率当作稳定理论阶数。

<figure id="fig-16">
  <img src="../../figures/01_advection_equation/analysis/02_sine_wave_quad_linearUpwind/all_N_comparison.png" alt="四边形线性迎风各分辨率场对比" width="720">
...
<figure id="fig-17">
  <img src="../../figures/01_advection_equation/cases/02_sine_wave_quad_linearUpwind/N80/field_comparison.png" alt="四边形线性迎风场对比 N80" width="720">
...
<figure id="fig-18">
  <img src="../../figures/01_advection_equation/cases/02_sine_wave_quad_linearUpwind/N80/diagonal_profile.png" alt="四边形线性迎风对角线剖面 N80" width="720">
...
<figure id="fig-19">
  <img src="../../figures/01_advection_equation/cases/02_sine_wave_quad_linearUpwind/N80/amplitude_history.png" alt="四边形线性迎风振幅历史 N80" width="720">
...
<figure id="fig-20">
  <img src="../../figures/01_advection_equation/cases/02_sine_wave_quad_linearUpwind/N80/cfl_history.png" alt="四边形线性迎风 CFL 历史 N80" width="720">
  <figcaption>图 20：四边形线性迎风 CFL 历史 N80</figcaption>
</figure>

CFL 图（[图 20](#fig-20)）与一阶迎风基本一致，说明两种格式使用了相同的时间步控制条件。
线性迎风的
...
<figure id="fig-21">
  <img src="../../figures/01_advection_equation/analysis/03_sine_wave_tri_linearUpwind/convergence_errors.png" alt="三角形线性迎风误差收敛曲线" width="720">
...
<figure id="fig-22">
  <img src="../../figures/01_advection_equation/analysis/03_sine_wave_tri_linearUpwind/convergence_order.png" alt="三角形线性迎风收敛阶" width="720">
  <figcaption>图 22：三角形线性迎风收敛阶</figcaption>
</figure>

观察阶（[图 22](#fig-22)）为 `1.498497`、`1.305666` 和 `1.087355`。粗网格段出现
高于一阶的局部斜率，不能直接当作稳定超一阶结论；细网格的 `1.087355` 更能代表
当前空间—时间联合误差的渐近趋势，也不能据此单独宣称空间离散达到二阶。

<figure id="fig-23">
  <img src="../../figures/01_advection_equation/analysis/03_sine_wave_tri_linearUpwind/all_N_comparison.png" alt="三角形线性迎风各分辨率场对比" width="720">
...
<figure id="fig-24">
  <img src="../../figures/01_advection_equation/cases/03_sine_wave_tri_linearUpwind/N80/field_comparison.png" alt="三角形线性迎风场对比 N80" width="720">
...
<figure id="fig-25">
  <img src="../../figures/01_advection_equation/cases/03_sine_wave_tri_linearUpwind/N80/diagonal_profile.png" alt="三角形线性迎风对角线剖面 N80" width="720">
...
<figure id="fig-26">
  <img src="../../figures/01_advection_equation/cases/03_sine_wave_tri_linearUpwind/N80/amplitude_history.png" alt="三角形线性迎风振幅历史 N80" width="720">
...
<figure id="fig-27">
  <img src="../../figures/01_advection_equation/cases/03_sine_wave_tri_linearUpwind/N80/cfl_history.png" alt="三角形线性迎风 CFL 历史 N80" width="720">
  <figcaption>图 27：三角形线性迎风 CFL 历史 N80</figcaption>
</figure>

CFL 历史（[图 27](#fig-27)）的最大值不超过 `0.2`，因此该组误差降低主要来自空间插值
格式，而不是时间步条件发生变化。
...
这里的 `cycleL1AgainstInitial` 定义为
`sum(|T_final-T_initial|V)/sum(|T_initial|V)`，表示一圈旋转后数值场与同一网格
初始场之间的体积加权返回差异，用于观察形状变形和数值耗散。它不是严格的解析解
误差，也不能直接作为收敛阶；此外，复杂初始轮廓是根据题目图形进行参数化重构，
因此相关结论限于当前重构轮廓。六组最终场、等值线和 CFL 图的图号范围为
[图 28](#fig-28)至[图 45](#fig-45)。
...
<figure id="fig-28">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_quad_upwind/N50/field_comparison.png" alt="四边形旋转场对比 N50" width="720">
  <figcaption>图 28：四边形旋转场对比 N50</figcaption>
</figure>

<figure id="fig-29">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_quad_upwind/N50/contour_final.png" alt="四边形旋转最终等值线 N50" width="720">
  <figcaption>图 29：四边形旋转最终等值线 N50</figcaption>
</figure>

<figure id="fig-30">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_quad_upwind/N50/cfl_history.png" alt="四边形旋转 CFL 历史 N50" width="720">
  <figcaption>图 30：四边形旋转 CFL 历史 N50</figcaption>
</figure>

$N=50$ 时最终最大值为 `0.515869`，`cycleL1AgainstInitial=1.305504`。最终等值线仍
围绕正确的旋转中心和总体位置分布，但切口、圆盘边缘和峰值都明显变钝，如[图 28](#fig-28)和[图 29](#fig-29)
...
<figure id="fig-31">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_quad_upwind/N100/field_comparison.png" alt="四边形旋转场对比 N100" width="720">
  <figcaption>图 31：四边形旋转场对比 N100</figcaption>
</figure>

<figure id="fig-32">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_quad_upwind/N100/contour_final.png" alt="四边形旋转最终等值线 N100" width="720">
  <figcaption>图 32：四边形旋转最终等值线 N100</figcaption>
</figure>

<figure id="fig-33">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_quad_upwind/N100/cfl_history.png" alt="四边形旋转 CFL 历史 N100" width="720">
...
辨认；[图 33](#fig-33)中的最大 CFL 仍为 `0.2`，所以改善来自空间分辨率，而不是时间步
条件变化。

#### 四边形，N=200

<figure id="fig-34">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_quad_upwind/N200/field_comparison.png" alt="四边形旋转场对比 N200" width="720">
  <figcaption>图 34：四边形旋转场对比 N200</figcaption>
</figure>

<figure id="fig-35">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_quad_upwind/N200/contour_final.png" alt="四边形旋转最终等值线 N200" width="720">
  <figcaption>图 35：四边形旋转最终等值线 N200</figcaption>
</figure>

<figure id="fig-36">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_quad_upwind/N200/cfl_history.png" alt="四边形旋转 CFL 历史 N200" width="720">
...
<figure id="fig-37">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_tri_upwind/N50/field_comparison.png" alt="三角形旋转场对比 N50" width="720">
  <figcaption>图 37：三角形旋转场对比 N50</figcaption>
</figure>

<figure id="fig-38">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_tri_upwind/N50/contour_final.png" alt="三角形旋转最终等值线 N50" width="720">
  <figcaption>图 38：三角形旋转最终等值线 N50</figcaption>
</figure>

<figure id="fig-39">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_tri_upwind/N50/cfl_history.png" alt="三角形旋转 CFL 历史 N50" width="720">
...
<figure id="fig-40">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_tri_upwind/N100/field_comparison.png" alt="三角形旋转场对比 N100" width="720">
  <figcaption>图 40：三角形旋转场对比 N100</figcaption>
</figure>

<figure id="fig-41">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_tri_upwind/N100/contour_final.png" alt="三角形旋转最终等值线 N100" width="720">
  <figcaption>图 41：三角形旋转最终等值线 N100</figcaption>
</figure>

<figure id="fig-42">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_tri_upwind/N100/cfl_history.png" alt="三角形旋转 CFL 历史 N100" width="720">
...
<figure id="fig-43">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_tri_upwind/N200/field_comparison.png" alt="三角形旋转场对比 N200" width="720">
  <figcaption>图 43：三角形旋转场对比 N200</figcaption>
</figure>

<figure id="fig-44">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_tri_upwind/N200/contour_final.png" alt="三角形旋转最终等值线 N200" width="720">
  <figcaption>图 44：三角形旋转最终等值线 N200</figcaption>
</figure>

<figure id="fig-45">
  <img src="../../figures/01_advection_equation/cases/04_solid_rotation_tri_upwind/N200/cfl_history.png" alt="三角形旋转 CFL 历史 N200" width="720">
...
目前可以直接核验完整 `checkMesh` 与求解器日志的是四边形一阶迎风组的四个分辨率；
其中 $N=80$ 的网格包含 `6400` 个六面体单元，最大长宽比为 `1`，最大非正交性为
`0`，最大偏斜度约为 $5.33\times10^{-14}$，日志给出 `Mesh OK`。其他实验组的
当前快照主要保留 `summary.json`、CSV 和图片，因此其 `meshOK`、`solverEnded` 等
状态应理解为汇总数据记录，而不是逐一可复核的完整运行日志。

一阶迎风结果没有出现明显的负值或超过初始最大值的非物理过冲。其主要误差表现为峰值
衰减和界面变宽，即数值耗散，而不是振荡或发散。

上述定量判断来自各分辨率目录中的 `summary.json`、`time_history.csv`、`field_data.csv`
和 `error_field.csv`；其中完整日志可复核的范围以本节前述说明为准，而不是只根据图片
目测得到。
...
6. 现有汇总数据表明通量守恒和 CFL 控制正常；
7. 三角形与四边形不能只按相同的 $N$ 进行公平精度比较。

需要特别说明的是，`cycleL1AgainstInitial` 随网格加密下降，说明当前重构轮廓的一圈
返回差异减小，但该量不是严格意义上的解析解收敛阶。由于固体旋转初始轮廓主要依据
题目图形参数化重构，后续若要给出更强的定量结论，还应补充原始轮廓参数、统一的解析
旋转误差定义以及更系统的格式对比。
...
[evidence_index.md](evidence_index.md)。该索引同时标注了当前保留的完整运行日志范围，
以避免将数据级汇总误写为完整运行证据。
