...
```bash
test -x build/bin/explicitAdvectionFoamStudent
echo $?
```
...
data/cases/01_sine_wave_quad/
figures/cases/01_sine_wave_quad/
...
```text
scripts/configs/04_solid_rotation_quad_linearUpwind.json
```
...
```bash
python3 scripts/run_study.py \
    --config scripts/configs/04_solid_rotation_quad_linearUpwind.json \
    --resolutions 50 \
    --prepare-only \
    --overwrite
```
...
```bash
cat cases/04_solid_rotation_quad_linearUpwind/N50/system/fvSchemes
```
...
```bash
python3 scripts/run_study.py \
    --config scripts/configs/<config>.json \
    --resolutions 50,100 \
    --prepare-only \
    --overwrite
```
...
```text
data/analysis/<caseName>/raw_results.csv
