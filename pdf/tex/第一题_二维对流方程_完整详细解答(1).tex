...
\subsection{两道题统一采用的面定向与残差记号}

为使第一题的对流离散与第二题的扩散离散能够逐式对照，本文不再用某个当前
单元的局部字母 $P$、$N$ 表示共享面两侧，而对每个内部面 $f$ 固定全局方向
$O_f\to N_f$。统一记号如下：
\begin{longtable}{L{0.25\linewidth}L{0.64\linewidth}}
\toprule
记号 & 含义\\
\midrule
\endfirsthead
...
\midrule
\endhead
$c,\Omega_c,V_c$ & 当前控制体、控制体区域及体积\\
$\mathcal F_c$ & 控制体 $c$ 的全部面\\
$O_f,N_f$ & 内部面 $f$ 的owner单元与neighbour单元\\
$\bm n_f$ & 从 $O_f$ 指向 $N_f$ 的全局单位法向量\\
$\Sf_f=|S_f|\bm n_f$ & 按 $O_f\to N_f$ 定向的全局有向面积向量\\
$\sigma_{cf}$ & 面 $f$ 相对于控制体 $c$ 的关联符号：$c=O_f$ 时为 $+1$，$c=N_f$ 时为 $-1$\\
$\Sf_{cf}=\sigma_{cf}\Sf_f=|S_f|\bm n_{cf}$ & 面 $f$ 相对于控制体 $c$ 的局部外向面积向量\\
$F_f=\bm u_f\cdot\Sf_f$ & 按 $O_f\to N_f$ 定向的体积通量\\
$H_f=F_f\phi_f$ & 按 $O_f\to N_f$ 定向的数值标量通量\\
$Q_c$ & 控制体 $c$ 的未归一化净外向通量和\\
$R_c=Q_c/V_c$ & 控制体 $c$ 的体积归一化空间残差\\
\bottomrule
\end{longtable}
...
\begin{equation}
  ...
  \label{eq:unified_orientation}
\end{equation}
因此一个内部面只计算一次 $H_f$，再以相反符号装配到两个单元：
\begin{equation}
  Q_{O_f}\mathrel{+}=H_f,
  \qquad
  Q_{N_f}\mathrel{-}=H_f.
  \label{eq:unified_face_assembly_intro}
\end{equation}
这套记号不再引入额外的“相邻单元映射”；共享面的两侧始终明确写成
$O_f$ 和 $N_f$。
...
\begin{equation}
  ...
  =
  \frac1{V_c}
  \int_{\Omega_c}\phi(\xf,t)\dd V.
  \label{eq:cell_average}
\end{equation}

二维问题中 $V_c$ 是单元面积；OpenFOAM通常使用单位厚度或薄层三维网格，因此程序仍以体积形式存储。

\subsection{控制体积分}

对式\eqref{eq:governing}在 $\Omega_c$ 上积分：
...
\begin{equation}
  ...
  \bm u\phi\cdot\bm n_c\dd S.
\end{equation}

将单元边界拆为面集合 $\mathcal F_c$，采用面中心积分：
...
\begin{equation}
  ...
  (\bm u_f\cdot\Sf_{cf})\phi_f.
\end{equation}

对内部面使用式\eqref{eq:unified_orientation}的全局定向，定义
...
\begin{equation}
  ...
  H_f=F_f\phi_f.
\end{equation}
边界面相对于其所属控制体的向外标量通量记为 $H_{cb}$，并定义
\begin{equation}
  B_c:=\sum_{b\in\mathcal F_c^{\mathrm{bnd}}}H_{cb}.
\end{equation}
内部面通量的未归一化代数和与体积归一化残差分别为
\begin{equation}
  \boxed{
  Q_c(\bm\phi)
  ...
  \label{eq:advection_residual_definition}
\end{equation}
于是半离散形式为

\begin{equation}
  \boxed{
  V_c\frac{\dd\phi_c}{\dd t}+Q_c(\bm\phi)=0,
  \qquad
  \frac{\dd\phi_c}{\dd t}+R_c(\bm\phi)=0
  }.
...
未知量 $\phi_c$ 是单元平均，但输运发生在面上，因此式\eqref{eq:semidiscrete}需要 $\phi_f$。不同空间格式的本质就是如何从周围单元值重构 $\phi_f$。

\subsection{一阶迎风}

按统一的 $O_f\to N_f$ 面方向定义体积通量
...
$F_f\geq0$ 表示信息从owner流向neighbour，所以面值取 $\phi_{O_f}$；$F_f<0$ 表示信息从neighbour流向owner，所以面值取 $\phi_{N_f}$。令
\begin{equation}
  F_f^+=\max(F_f,0),
  \qquad
  F_f^-=\min(F_f,0),
...
\begin{equation}
  ...
  \label{eq:upwind_numerical_flux}
\end{equation}
该面只计算一次，并按
\begin{equation}
  Q_{O_f}\mathrel{+}=H_f^{\mathrm{adv}},
  \qquad
  Q_{N_f}\mathrel{-}=H_f^{\mathrm{adv}}
\end{equation}
装配。
...
对不可压缩速度场，控制体 $c$ 的局部外向体积通量
$F_{cf}:=\sigma_{cf}F_f$ 满足

\begin{equation}
  \sum_{f\in\mathcal F_c}F_{cf}=0.
\end{equation}

因此保持一阶迎风单调性的局部条件可以直接写为

\begin{equation}
  \boxed{
  \frac{\Delta t}{V_c}
  \sum_{f\in\mathcal F_c}\max(F_{cf},0)\leq1.}
\label{eq:convex_update}
\end{equation}
在该条件下，$\phi_c^{n+1}$ 可写成当前单元旧值与各入流面上游单元旧值的
凸组合。对于 $c=O_f$ 的入流面，$F_f<0$，上游值为 $\phi_{N_f}$；对于
$c=N_f$ 的入流面，$F_f>0$，上游值为 $\phi_{O_f}$。因此不需要任何
额外邻接映射。如果初值和入流边界位于区间 $[m,M]$，数值解也不会轻易
越出该区间。这解释了一阶迎风的有界性。
...
\begin{equation}
  ...
  =
  \frac{\Delta t}{2V_c}
  \sum_{f\in\mathcal F_c}|F_f|.
\end{equation}

不可压缩条件下，入流绝对通量之和等于出流通量之和，因此

\begin{equation}
  \frac12\sum_{f\in\mathcal F_c}|F_f|
  =
  \sum_{f\in\mathcal F_c}\max(F_{cf},0).
\end{equation}
...
三角形或非均匀网格中不能仅用统一 $h$ 估算，必须根据每个单元的 $V_c$ 与面通量计算局部 $\Co_c$。
...
\begin{equation}
  \phi_c
  =
  \frac1{V_c}\int_{\Omega_c}\phi\dd V.
\end{equation}
...
设数值解与解析解分别为 $\phi_c^{\mathrm{num}}$ 和 $\phi_c^{\mathrm{exact}}$。体积加权相对误差可定义为
...
\begin{equation}
  ...
  \frac{
  \sum_c
  |\phi_c^{\mathrm{num}}-\phi_c^{\mathrm{exact}}|V_c
  }{
  \sum_c
  |\phi_c^{\mathrm{exact}}|V_c
  },
\end{equation}
...
\begin{equation}
  ...
  \frac{
  \sum_c
  (\phi_c^{\mathrm{num}}-\phi_c^{\mathrm{exact}})^2V_c
  }{
  \sum_c
  (\phi_c^{\mathrm{exact}})^2V_c
  }
  \right]^{1/2},
\end{equation}
...
\begin{equation}
  \phi_{\max}(t)=\max_c\phi_c(t),
  \qquad
  \phi_{\min}(t)=\min_c\phi_c(t).
\end{equation}
...
while t < endTime:
    ...
    choose dt so that max(Co_c) <= CoMax
    shorten the last dt if t + dt > endTime

    reconstruct phi_f on every internal and boundary face
    initialise Q_c = 0 for every cell
    for every internal face:
        H_f = F_f * phi_f
        Q_{O_f} += H_f
        Q_{N_f} -= H_f
    add boundary outward fluxes to Q_c

    normalise R_c = Q_c / V_c
    update every cell synchronously:
        phi_c <- phi_c - dt * R_c
...
\midrule
\endhead
单元标量 $\phi_c$ & \texttt{volScalarField}\\
面通量 $F_f$ & \texttt{surfaceScalarField}\\
单元体积 $V_c$ & \texttt{mesh.V()}\\
