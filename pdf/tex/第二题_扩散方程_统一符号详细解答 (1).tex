...
$Q_c=\sum_{f\in\mathcal F_c^{\mathrm{int}}}\sigma_{cf}H_f+B_c$ & 控制体 $c$ 的未归一化净外向通量和\\
...
\begin{equation}
  \boxed{
  Q_c(\bm\phi)
  :=\sum_{f\in\mathcal F_c^{\mathrm{int}}}\sigma_{cf}H_f+B_c},
...
\begin{equation}
  ...
  =\frac{1}{V_c}
  \left(\sum_{f\in\mathcal F_c^{\mathrm{int}}}\sigma_{cf}H_f+B_c\right)}.
...
\begin{equation}
  ...
  \sigma_{cf}D_f(\phi_{O_f}-\phi_{N_f})+B_c,}
  \label{eq:orth_Q}
\end{equation}
从而
\begin{equation}
...
\end{equation}
其中 $B_c$ 汇总边界面的向外扩散通量。对每一个内部面，owner方程得到
...
\begin{equation}
  ...
  \left[D_f(\phi_{O_f}-\phi_{N_f})-C_f\right]+B_c,
  \qquad
...
\begin{equation}
  ...
  \sum_{f\in\mathcal F_c^{\mathrm{int}}}
  \sigma_{cf}\left[
  D_f^n(\phi_{O_f}^n-\phi_{N_f}^n)-C_f^n\right]+B_c^n
  \right\}.
...
\begin{equation}
  ...
  \qquad
  Q_c^n
  =\sum_{f\in\mathcal F_c^{\mathrm{int}}}\sigma_{cf}H_f^n+B_c^n,
  \qquad
