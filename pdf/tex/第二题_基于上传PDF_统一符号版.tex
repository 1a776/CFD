...
$\Sf_{cf}$、全局owner--neighbour面积向量 $\Sf_f$、面通量 $H_f$、未归一化
面通量和 $Q_c$ 以及体积归一化残差 $R_c=Q_c/V_c$。扩散方程所特有的中心
连线、扩散传递系数和非正交修正另行定义。
...
$\Sf_{cf}=|S_f|\bm n_{cf}$ & 面 $f$ 相对于控制体 $c$ 的局部有向面积向量\\
$O_f,N_f$ & 内部面 $f$ 的owner单元与neighbour单元\\
$\bm n_f$ & 从 $O_f$ 指向 $N_f$ 的全局单位法向量\\
$\Sf_f=|S_f|\bm n_f$ & 从 $O_f$ 指向 $N_f$ 的全局有向面积向量\\
$\sigma_{cf}$ & 面 $f$ 相对于控制体 $c$ 的关联符号：$c=O_f$ 时为 $+1$，$c=N_f$ 时为 $-1$\\
$\bm d_f=\xf_{N_f}-\xf_{O_f}$ & 从owner中心指向neighbour中心的向量\\
$d_f=|\bm d_f|$ & owner与neighbour单元中心之间的距离\\
$\phi_c$ & $\phi$ 在控制体 $c$ 中的平均值\\
$D_f$ & 面 $f$ 的离散扩散传递系数\\
$H_f$ & 按 $O_f\to N_f$ 定向的全局数值扩散通量\\
$H_{cf}=\sigma_{cf}H_f$ & 内部面 $f$ 相对于控制体 $c$ 的局部向外通量\\
$Q_c=\sum_{f\in\mathcal F_c^{\mathrm{int}}}\sigma_{cf}H_f+Q_c^{\mathrm{bnd}}$ & 控制体 $c$ 的未归一化净外向通量和\\
$R_c=Q_c/V_c$ & 控制体 $c$ 的体积归一化扩散残差（离散散度）\\
\bottomrule
\end{longtable}

\source{\Rtwo，第1.3.1节及式(1.24)--(1.26)：面面积向量、owner-neighbour寻址和共享面通量装配；\Rone，第8章：扩散离散中的控制体、相邻单元和面系数。}

对任意内部面 $f$，全局面积向量 $\Sf_f$ 的方向固定为从 $O_f$ 指向 $N_f$；
相对于两个控制体的局部外法向面积向量满足
\begin{equation}
  \boxed{
  \sigma_{cf}=
  \begin{cases}
  ...
  \end{cases}
  \qquad
  \Sf_{cf}=\sigma_{cf}\Sf_f=|S_f|\bm n_{cf}},
  \label{eq:local_global_area_vectors}
\end{equation}
并且 $|S_f|=|\Sf_f|=|\Sf_{cf}|$ 表示面 $f$ 的面积（二维单位厚度下为边长）。
按同一定向定义内部面的全局扩散通量 $H_f$，则
\begin{equation}
  \boxed{
  H_{cf}=\sigma_{cf}H_f,
  \qquad
...
\end{equation}
为了与第一题完全一致，首先按全局方向 $O_f\to N_f$ 定义内部面的数值扩散通量
\begin{equation}
  \boxed{
  H_f:=\bm J_{\phi,D,f}\cdot\Sf_f
...
\begin{equation}
  Q_c^{\mathrm{bnd}}:=\sum_{b\in\mathcal F_c^{\mathrm{bnd}}}H_{cb}.
...
\begin{equation}
  \boxed{
  Q_c(\bm\phi)
  :=\sum_{f\in\mathcal F_c^{\mathrm{int}}}\sigma_{cf}H_f+Q_c^{\mathrm{bnd}}},
  \label{eq:diffusive_face_sum}
\end{equation}
再除以控制体体积，定义体积归一化扩散残差
\begin{equation}
  \boxed{
  R_c(\bm\phi):=\frac{Q_c(\bm\phi)}{V_c}
  =\frac{1}{V_c}
  \left(\sum_{f\in\mathcal F_c^{\mathrm{int}}}\sigma_{cf}H_f+Q_c^{\mathrm{bnd}}\right)}.
...
$V_c\mathrm d\phi_c/\mathrm dt+Q_c=0$ 以及
$\mathrm d\phi_c/\mathrm dt+R_c=0$ 具有完全相同的守恒结构，区别仅在于
第一题的 $H_f$ 是对流通量，而本题的 $H_f$ 是扩散通量。这里 $Q_c$ 是装配
过程中的未归一化中间量，$R_c=Q_c/V_c$ 才是最终空间残差。所谓半离散，是指
...
对每个内部面 $f$，直接使用owner--neighbour几何量
\begin{equation}
...
\end{equation}
假设 $\phi$ 在两个单元中心之间线性变化，沿 $\ef_f$ 的方向导数为
\begin{equation}
...
\begin{align}
  ...
  &\approx\frac{|S_f|}{d_f}(\phi_{N_f}-\phi_{O_f}).
    \label{eq:orth_grad_2}
\end{align}
定义正的面扩散传递系数
\begin{equation}
  \boxed{D_f=\mu_f\frac{|S_f|}{d_f}},
  \label{eq:D_orth}
\end{equation}
根据式\eqref{eq:diffusive_numerical_flux}的全局通量约定，该面的数值扩散通量为
\begin{equation}
  \boxed{
...
将式\eqref{eq:orth_face_contribution}按式\eqref{eq:global_face_assembly}装配：
\begin{equation}
  \boxed{
  Q_c(\bm\phi)
  =\sum_{f\in\mathcal F_c^{\mathrm{int}}}
  \sigma_{cf}D_f(\phi_{O_f}-\phi_{N_f})+Q_c^{\mathrm{bnd}},}
  \label{eq:orth_Q}
\end{equation}
从而
\begin{equation}
  \boxed{
  V_c\frac{\mathrm d\phi_c}{\mathrm dt}+Q_c(\bm\phi)=0,
  \qquad
  \frac{\mathrm d\phi_c}{\mathrm dt}+\frac{Q_c(\bm\phi)}{V_c}=0,}
  \label{eq:orth_semidiscrete}
\end{equation}
其中 $Q_c^{\mathrm{bnd}}$ 汇总边界面的向外扩散通量。对每一个内部面，owner方程得到
$+D_f(\phi_{O_f}-\phi_{N_f})$，neighbour方程得到其相反数。因此同一个面
系数 $D_f$ 只计算一次。以面为单位的两侧贡献为
\begin{equation}
  \begin{aligned}
  Q_{O_f}&\mathrel{+}=D_f\phi_{O_f}-D_f\phi_{N_f},\\
  Q_{N_f}&\mathrel{+}=-D_f\phi_{O_f}+D_f\phi_{N_f}.
  \end{aligned}
  \label{eq:orth_expanded}
\end{equation}
两式相加严格为零，这既给出对角系数与非对角系数的符号结构，也直接证明内部
面守恒；常数场 $\phi_{O_f}=\phi_{N_f}$ 时，该面通量为零，这正是参考书所述
“离散系数和为零”的性质。
...
题目要求在三角形和四边形网格上验证。一般非结构网格中，$\Sf_f$ 与
$\bm d_f$ 不平行，因此不能直接把整个面法向梯度写成
$(\phi_{N_f}-\phi_{O_f})/d_f$。参考书将全局有向面积向量分解为
\begin{equation}
  \boxed{\Sf_f=\Ef_f+\Tf_f},
  \qquad
  \Ef_f\parallel\bm d_f,
...
\begin{align}
  ...
  &\approx
    \frac{|\Ef_f|}{d_f}(\phi_{N_f}-\phi_{O_f})
    +(\nabla\phi)_f\cdot\Tf_f.
...
内部面值采用线性插值：
\begin{equation}
  \phi_f=g_{O_f f}\phi_{O_f}+g_{N_f f}\phi_{N_f},
  \qquad g_{O_f f}+g_{N_f f}=1,
...
\begin{equation}
  \boxed{
  Q_c(\bm\phi)
  =\sum_{f\in\mathcal F_c^{\mathrm{int}}}\sigma_{cf}
  \left[D_f(\phi_{O_f}-\phi_{N_f})-C_f\right]+Q_c^{\mathrm{bnd}},
  \qquad
  \frac{\mathrm d\phi_c}{\mathrm dt}+\frac{Q_c}{V_c}=0}.
...
\begin{equation}
  g_b(t)=\nabla\phi_{\mathrm{exact}}(\xf_b,t)\cdot\bm n_{cb}.
...
\begin{equation}
  ...
  \sum_{f\in\mathcal F_c^{\mathrm{int}}}
  \sigma_{cf}\left[
  D_f^n(\phi_{O_f}^n-\phi_{N_f}^n)-C_f^n\right]+(Q_c^{\mathrm{bnd}})^n
  \right\}.
...
当 $C_f=0$ 时，每个内部面首先计算
\begin{equation}
  H_f^n=D_f(\phi_{O_f}^n-\phi_{N_f}^n),
\end{equation}
然后对该面的两侧分别产生更新量
\begin{equation}
  \boxed{
  \Delta\phi_{O_f}
  =-\frac{\Delta t}{V_{O_f}}H_f^n,
  \qquad
  \Delta\phi_{N_f}
  =+\frac{\Delta t}{V_{N_f}}H_f^n.}
  \label{eq:explicit_coeff_form}
\end{equation}
所有内部面和边界面贡献累加完毕后再同步更新。该写法只使用 $O_f,N_f$，并且
清楚显示同一面向两个单元提供符号相反的守恒贡献。
...
\begin{equation}
  ...
  \min_c
  \frac{V_c}{
  \displaystyle\sum_{f\in\mathcal F_c^{\mathrm{int}}}D_f
  +\displaystyle\sum_{b\in\mathcal F_c^{\mathrm{bnd}}}D_b}}.
...
在时间 $t^n$，先更新边界条件，再按式\eqref{eq:safe_dt}计算 $\Delta t$。根据式\eqref{eq:gauss_cell_gradient}和式\eqref{eq:face_gradient_interpolation}计算单元梯度及面梯度，并将所有单元未归一化面通量和初始化为零：
\begin{equation}
  Q_c^n=0.
...
然后以相反符号装配到两个相邻控制体：
\begin{equation}
  Q_{O_f}^n\mathrel{+}=H_f^n,
  \qquad
  Q_{N_f}^n\mathrel{-}=H_f^n.
...
Neumann边界面使用
\begin{equation}
  H_{cb}^n=-\mu_bg_b^n|S_b|.
  \label{eq:algorithm_neumann}
\end{equation}
将边界贡献加入 $Q_c^n$ 后，先按第一题的定义进行体积归一化
\begin{equation}
  \boxed{R_c^n=\frac{Q_c^n}{V_c}},
  \label{eq:algorithm_normalize_residual}
\end{equation}
再对全部控制体同步进行向前Euler更新：
\begin{equation}
  \boxed{
  \phi_c^{n+1}
  =\phi_c^n-\Delta t\,R_c^n}.
...
\begin{equation}
  ...
  +Q_c(\bm\phi)=0,
  \qquad
  \frac{\mathrm d\phi_c}{\mathrm dt}+R_c(\bm\phi)=0,
  \qquad
  R_c=\frac{Q_c}{V_c}.}
...
\begin{equation}
  ...
  \qquad
  Q_c^n
  =\sum_{f\in\mathcal F_c^{\mathrm{int}}}\sigma_{cf}H_f^n+(Q_c^{\mathrm{bnd}})^n,
  \qquad
  R_c^n=\frac{Q_c^n}{V_c}.}
...
\begin{equation}
  ...
  +\sum_{b\in\mathcal F_c^{\mathrm{bnd}}}D_b}}.
