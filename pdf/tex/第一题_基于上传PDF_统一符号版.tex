...
\ifusectex\else
  ...
  \IfFontExistsTF{Noto Sans CJK SC}{%
    ...
    \setmainfont[Path=\localcjkfontpath,AutoFakeBold=2,AutoFakeSlant=0.2]{NotoSansCJKsc-Regular.otf}
    \setsansfont[Path=\localcjkfontpath,AutoFakeBold=2,AutoFakeSlant=0.2]{NotoSansCJKsc-Regular.otf}
  }
...
$\sigma_{cf}$ & 面 $f$ 对控制体 $c$ 的关联符号：$c=O_f$ 时为 $+1$，$c=N_f$ 时为 $-1$\\
$F_f=\bm u_f\cdot\bm S_f$ & 面速度通量\\
$H_f=F_f\phi_f$ & 标量通过面的数值对流通量\\
$Q_c$ & 控制体 $c$ 的未除体积面通量代数和\\
$R_c=Q_c/V_c$ & 控制体 $c$ 的体积归一化对流残差（离散散度）\\
\bottomrule
\end{longtable}

对每个内部面统一规定全局方向 $O_f\to N_f$，于是
\begin{equation}
  \boxed{
  \sigma_{cf}=
  \begin{cases}
  ...
  \qquad
  F_{cf}=\sigma_{cf}F_f.}
  \label{eq:orientation_relation}
\end{equation}
同一个内部面通量只计算一次，并以相反符号装配到两侧控制体。
...
\begin{equation}
  \boxed{
  Q_c(\phiv):=\sum_{f\in\mathcal F_c}F_{cf}\phi_f,}
  \label{eq:flux_sum_general}
\end{equation}
再定义体积归一化的离散空间残差
\begin{equation}
  \boxed{
  R_c(\phiv):=\frac{Q_c(\phiv)}{V_c}
  =\frac{1}{V_c}\sum_{f\in\mathcal F_c}F_{cf}\phi_f.}
...
\begin{equation}
  \boxed{
  V_c\frac{\mathrm d\phi_c}{\mathrm dt}+Q_c(\phiv)=0,
  \qquad
...
\begin{equation}
  ...
  \label{eq:mol_global}
\end{equation}
式\eqref{eq:semidiscrete_general}--\eqref{eq:mol_global}中，空间散度已经转化为有限个面通量之和，而时间导数仍保持连续，因此它们就是题目要求的有限体积半离散形式。此时面值 $\phi_f$ 尚待空间插值格式封闭。
...
\begin{equation}
  \boxed{R_c(\phiv)=\frac{Q_c(\phiv)}{V_c}.}
  \label{eq:normalize_residual}
\end{equation}
因此，$Q_c$ 是装配过程中的中间量，$R_c$ 才是本文统一使用的最终空间残差。

\source{\Rtwo，式(1.26)：每个内部面的离散散度贡献对owner取加号、对neighbour取减号；\Rone，第5.2.1节式(5.8)--(5.10)后的说明：面通量形式使有限体积法具有守恒性。}

\subsection{封闭的迎风半离散方程}

不再引入额外的“相邻单元映射”，而直接用 $O_f,N_f$ 表示共享面两侧。边界面的局部向外对流通量代数和记为 $Q_c^{\mathrm{bnd}}$。由式\eqref{eq:upwind_flux_global}和关联符号 $\sigma_{cf}$，得到
\begin{equation}
  \boxed{
  Q_c(\phiv)
  =\sum_{f\in\mathcal F_c^{\mathrm{int}}}
  \sigma_{cf}
  \left(F_f^+\phi_{O_f}+F_f^-\phi_{N_f}\right)
  +Q_c^{\mathrm{bnd}}
  }.
  \label{eq:upwind_flux_sum}
\end{equation}
体积归一化残差及封闭的一阶迎风半离散方程为
\begin{equation}
  \boxed{
  R_c(\phiv)=\frac{Q_c(\phiv)}{V_c},
  \qquad
  \frac{\mathrm d\phi_c}{\mathrm dt}=-R_c(\phiv).}
  \label{eq:residual}
\end{equation}
这里对任意内部面均只出现 $O_f,N_f$，且同一个 $H_f$ 对owner与neighbour分别取 $+1$ 和 $-1$。

\source{\Rone，第5.2.3节：面通量代入有限体积方程；第11.2.4节：迎风面值；\Rtwo，式(1.23)与(1.26)：面插值后的离散散度及owner-neighbour装配。式\eqref{eq:upwind_flux_sum}--\eqref{eq:residual}是这些参考公式针对本题纯对流方程的直接组合。}

\subsection{离散质量守恒}

对全部控制体的式\eqref{eq:residual}乘以 $V_c$ 后求和：
\begin{equation}
...
\end{equation}
由于 $V_cR_c=Q_c$，内部面按照式\eqref{eq:owner_add}--\eqref{eq:neighbour_subtract}成对抵消，因此只剩边界通量：
\begin{equation}
  \frac{\mathrm d}{\mathrm dt}\sum_cV_c\phi_c
...
将显式迎风残差式\eqref{eq:residual}代入式\eqref{eq:euler_update_residual}：
\begin{equation}
  \boxed{
  \phi_c^{n+1}
  =\phi_c^n
...
\end{equation}
式\eqref{eq:fully_discrete}右端只含旧时间层的 $\phi_{O_f}^n$、$\phi_{N_f}^n$、$F_f^n$ 和边界通量贡献，所以可直接计算 $\phi_c^{n+1}$，无需组装或求解线性代数方程组。
...
为把局部系数形式仍然写成统一的owner--neighbour记号，定义相对于控制体 $c$ 的入流侧单元值
\begin{equation}
  \phi_{\mathrm{in},cf}^n=
  \begin{cases}
    \phi_{N_f}^n,&c=O_f,\ F_f^n<0,\\
    \phi_{O_f}^n,&c=N_f,\ F_f^n>0.
  \end{cases}
  \label{eq:incoming_cell_value}
\end{equation}
在周期边界或边界通量按同一迎风原则处理的情形下，将式\eqref{eq:fully_discrete}整理为
\begin{align}
  \phi_c^{n+1}
  =&\left[
    1-\frac{\Delta t}{V_c}
    \sum_{f\in\mathcal F_c}(F_{cf}^n)^+
  \right]\phi_c^n
  \nonumber\\
  &+\sum_{\substack{f\in\mathcal F_c\\F_{cf}^n<0}}
  \left[
    -\frac{\Delta t}{V_c}(F_{cf}^n)^-
  \right]\phi_{\mathrm{in},cf}^n.
...
\begin{equation}
  \sum_{f\in\mathcal F_c}F_{cf}^+
  =\frac12\sum_{f\in\mathcal F_c}|F_{cf}|,
...
\subsubsection*{步骤1：冻结旧时间层并清零未归一化通量和}
...
全部面通量计算完毕并将边界贡献加入 $Q_c^n$ 后，先进行体积归一化：
\begin{equation}
  \boxed{R_c^n=\frac{Q_c^n}{V_c}}.
  \label{eq:solver_normalize}
\end{equation}
再对所有控制体同时更新：
\begin{equation}
  \boxed{
...
\end{equation}
必须先完成所有 $Q_c^n$ 的装配与 $R_c^n=Q_c^n/V_c$ 的计算，再统一更新 $\phi_c^{n+1}$；若在面循环中直接覆盖 $\phi_c^n$，后续面会混用新旧时间层，便不再是式(13.7)定义的前向Euler格式。
...
\end{equation}
若在离散格式字典中为该散度项选择 \texttt{Gauss upwind}，则 \texttt{fvc::div} 通过一阶迎风面值形成当前时间层的离散散度场。前向Euler更新的概念表达为
\begin{equation}
  \boxed{
  \phi^{n+1}
...
\begin{equation}
  ...
  \sum_{f\in\mathcal F_c^{\mathrm{int}}}
  \sigma_{cf}
  \left[(F_f^n)^+\phi_{O_f}^n
  +(F_f^n)^-\phi_{N_f}^n\right]
  +(Q_c^{\mathrm{bnd}})^n
  \right\}
  }.
\end{equation}
