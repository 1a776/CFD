...
$\Sf_{cf}$、全局owner--neighbour面积向量 $\Sf_f$ 和面通量 $H_f$。
扩散方程所特有的中心连线、扩散传递系数和非正交修正另行定义。
与 OpenFOAM 求解器直接对应的空间残差不再引入未归一化的中间量，而是直接
定义为控制体上 $\nabla\cdot(\mu\nabla\phi)$ 的有限体积近似。
...
$H_{cf}=\sigma_{cf}H_f$ & 内部面 $f$ 相对于控制体 $c$ 的局部向外物理扩散通量\\
$H_{cb}$ & 边界面 $b$ 相对于所属控制体的向外物理扩散通量\\
$R_c$ & 控制体 $c$ 上 $\nabla\cdot(\mu\nabla\phi)$ 的体积归一化离散残差\\
\bottomrule
\end{longtable}
...
\begin{equation}
  ...
  +
  \sum_{b\in\mathcal F_c^{\mathrm{bnd}}}H_{cb}
  \right].}
  \label{eq:residual_symbol_definition}
\end{equation}
...
\begin{equation}
  ...
  H_{cf}=\sigma_{cf}H_f,
  \qquad
  R_{O_f}\mathrel{-}=\frac{H_f}{V_{O_f}},
  \qquad
  R_{N_f}\mathrel{+}=\frac{H_f}{V_{N_f}}.}
  \label{eq:global_face_assembly}
\end{equation}
这里 $H_f$ 是物理扩散通量，而 $R_c$ 是
$\nabla\cdot(\mu\nabla\phi)$ 的离散值，所以两者之间带有一个负号。
...
\end{equation}
边界面的局部向外通量记为 $H_{cb}$。根据物理扩散通量
$\bm J_{\phi,D}=-\mu\nabla\phi$，直接定义控制体 $c$ 上的空间残差：
\begin{equation}
  \boxed{
  R_c(\bm\phi)
  :=
...
\begin{equation}
  H_f=-\mu_f(\nabla\phi)_f\cdot\Sf_f,
\end{equation}
所以式\eqref{eq:diffusive_residual}正好是
\begin{equation}
  \boxed{
  R_c(\bm\phi)
  =
  \left[\nabla\cdot(\mu\nabla\phi)\right]_c.}
  \label{eq:diffusive_residual_laplacian}
\end{equation}
...
$V_c\mathrm d\phi_c/\mathrm dt-V_cR_c=0$ 以及
$\mathrm d\phi_c/\mathrm dt-R_c=0$ 具有相同的守恒结构，区别仅在于
第一题的 $H_f$ 是对流通量，而本题的 $H_f$ 是扩散通量。这里 $R_c$ 已经是
最终的体积归一化空间残差，不需要再经过额外的中间变量转换。所谓半离散，是指
...
\begin{equation}
  ...
  \sigma_{cf}D_f(\phi_{O_f}-\phi_{N_f})
  +
  \sum_{b\in\mathcal F_c^{\mathrm{bnd}}}H_{cb}
  \right].}
  \label{eq:orth_residual}
\end{equation}
从而
\begin{equation}
  \boxed{
...
\end{equation}
其中边界面通量直接进入 $R_c$，不再单独定义 $B_c$。对每一个内部面，
物理扩散通量为 $H_f=D_f(\phi_{O_f}-\phi_{N_f})$，因此同一个面系数
$D_f$ 只计算一次。以面为单位的两侧残差贡献为
\begin{equation}
  \begin{aligned}
  R_{O_f}&\mathrel{-}=\frac{D_f(\phi_{O_f}-\phi_{N_f})}{V_{O_f}},\\
  R_{N_f}&\mathrel{+}=\frac{D_f(\phi_{O_f}-\phi_{N_f})}{V_{N_f}}.
  \end{aligned}
  \label{eq:orth_expanded}
\end{equation}
乘以各自体积后，两侧对全局物理通量的贡献严格相反；常数场
$\phi_{O_f}=\phi_{N_f}$ 时，该面通量为零。这正是参考书所述“离散系数和为零”
的守恒性质。
...
\begin{equation}
  ...
  \right],
  \qquad
  \frac{\mathrm d\phi_c}{\mathrm dt}-R_c=0}.
...
\end{equation}
然后直接按体积归一化形式装配到两个相邻控制体：
\begin{equation}
  R_{O_f}^n\mathrel{-}=\frac{H_f^n}{V_{O_f}},
  \qquad
  R_{N_f}^n\mathrel{+}=\frac{H_f^n}{V_{N_f}}.
  \label{eq:owner_neighbour_assembly}
\end{equation}
乘以各自控制体体积后，同一个内部面在两个控制体中的物理通量贡献严格相反，
因此内部通量在全局求和时抵消，保证离散守恒。
...
边界贡献直接加入所属控制体的残差：
\begin{equation}
  R_{\operatorname{owner}(b)}^n
  \mathrel{-}=
  \frac{H_{cb}^n}{V_{\operatorname{owner}(b)}}.
  \label{eq:algorithm_boundary_residual}
\end{equation}
...
\end{lstlisting}
这段代码只表达离散结构。具体API需要与所安装的OpenFOAM 14字段类型和编译环境核对。
这里的 $R_c$ 直接对应
$\texttt{fvc::laplacian(mu,phi)}$ 返回的单元场：
\begin{equation}
  R_c
  =
  \left[\texttt{fvc::laplacian(mu,phi)}\right]_c
  =
  \left[\nabla\cdot(\mu\nabla\phi)\right]_c.
\end{equation}
因此更新式是 $\phi^{n+1}=\phi^n+\Delta tR_c^n$。关键原则仍是使用
...
$\phi_c^{\mathrm{exact}}$。严格按照原题附录A，采用体积加权归一化误差：
\begin{align}
  E(L_1)
  &=\frac{\sum_cV_c
...
$\phi_{ni}$、$\phi_{ei}$、$|\Omega_i|$ 完全对应，只是将单元索引统一写成 $c$。
...
\begin{equation}
  ...
  =-\frac{1}{V_c}
  \left[
  \sum_{f\in\mathcal F_c^{\mathrm{int}}}\sigma_{cf}H_f^n
  +
  \sum_{b\in\mathcal F_c^{\mathrm{bnd}}}H_{cb}^n
  \right].}
